...
def main():
    ...
    print(f"Profile: {len(all_skill_names)} skills, {len(agent_names)} agents, "
          f"{len(command_names)} commands, {len(rule_names)} rules, {len(mcp_names)} MCPs")
    ...
    # Generate hooks.json if hooks are specified
    if hook_names:
    ...
    print(f"  Skills: {stats['skills']}, Agents: {stats['agents']}, "
          f"Commands: {stats['commands']}, Rules: {stats['rules']}, MCP: {stats['mcp']}")
