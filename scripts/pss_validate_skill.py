#!/usr/bin/env python3
"""
Perfect Skill Suggester (PSS) Skill Validator
...
Usage:
    uv run python scripts/pss_validate_skill.py path/to/skill/
    uv run python scripts/pss_validate_skill.py path/to/skill/ --verbose
    uv run python scripts/pss_validate_skill.py path/to/skill/ --json
...
def validate_frontmatter(
    ...
    if not content.startswith("---"):
        report.info(
            "No YAML frontmatter found (optional but recommended)", "SKILL.md"
        )
        return None
...
def validate_context_field(
    ...
    if context not in VALID_CONTEXT_VALUES:
        report.critical(
            ...
            f"Valid values: {VALID_CONTEXT_VALUES}",
            "SKILL.md",
        )
        return
...
def validate_agent_field(frontmatter: dict[str, Any], report: ValidationReport) -> None:
    ...
    if "agent" not in frontmatter:
        ...
        if frontmatter.get("context") == "fork":
            report.info(
                "'agent' not specified with context: fork "
    ...
    if agent in BUILTIN_AGENT_TYPES:
        ...
        report.info(
            f"'agent' value '{agent}' is not a built-in type "
            "(may be custom from .claude/agents/)",
            "SKILL.md",
        )
...
def validate_skill_content(content: str, report: ValidationReport) -> None:
    ...
    if re.search(r"^\d+\.", body, re.MULTILINE) or "```bash" in body.lower():
        if "$ARGUMENTS" not in content:
            report.info(
                "Task-oriented skill without $ARGUMENTS placeholder "
                "(arguments will be appended automatically)",
                "SKILL.md",
            )
