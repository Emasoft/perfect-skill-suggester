...
def validate_name_field(
    ...
    validate_component_name(
        ...
        "skill",
        report,
        directory_name=unicodedata.normalize("NFKC", skill_dir_name)
...
def validate_token_budget(content: str, body: str, report: ValidationReport) -> None:
    ...
    if char_count > MAX_CHAR_COUNT_ERROR:
        ...
        report.passed(
            f"SKILL.md character count OK ({char_count} chars)",
            "SKILL.md",
            category="Token Budget",
        )
...
def validate_reference_files(skill_path: Path, report: ValidationReport) -> None:
    ...
    for ref_file in refs_dir.glob("*.md"):
        try:
            ...
            has_toc_early = bool(
                re.search(
                    r"(?im)^##\s*(contents|table\s+of\s+contents|toc|index)(\s|$)", head
                )
...
def print_results(report: ValidationReport, verbose: bool = False) -> None:
    ...
    for r in report.results:
        ...
        if (
            isinstance(r, ComprehensiveValidationResult)
            and r.category
            and r.category not in categories_seen
        ):
        ...
        "results": [
            {
                ...
                "line": r.line,
                "category": r.category
                if isinstance(r, ComprehensiveValidationResult)
                else None,
            }
            for r in report.results
        ],
