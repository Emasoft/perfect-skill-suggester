...
description: "PSS suggested an agent from a different project / why is it suggesting something the harness never loaded / unrelated project's skills scored against my prompt / cross-project suggestion leak / is the PSS index bound to one project / does a mid-session /cd rebind the element inventory / do I need a CwdChanged hook to rescope suggestions / the index still has the old project's elements after cd / how many projects are in the skill index / what does --all-projects do to the index / what is the bare 'project' source / project: vs local: source format / why does the cwd project have no slug / seen_project_paths skips the cwd / how do I filter candidates by their origin project / where is the suggest-time origin filter / is_foreign_project_element / agents from another checkout keep appearing / the TRDD prescribed a fix that turned out wrong / acceptance criteria all pass but the bug remains / I implemented what the card said and nothing changed / should I trust a card's diagnosis or re-measure the premise / shipped a no-op fix / draining the board blindly / confident wrong fix / my test passes when I delete the guard / tautological test asserting trim is_empty / testing a copy of the logic instead of the logic / how do I mutation test a guard / is my test actually a tripwire / the guard has no regression coverage / fail open vs fail closed / all project-scoped elements dropped at once / my project's own skills vanish from a subdirectory / suggestions work at the repo root but not in src/ / cd into a subfolder and lose the project's agents / four bugs in one feature / I keep patching inputs and it keeps breaking / is cwd a project root or is it inside one / walk up to the owning project root"
ocd: 2026-08-29
lmd: 2026-08-29
...
path must fail closed (one missing suggestion beats restoring the leak). [^3]
...
[^3]: [id: ATOM-1T6F-GDST, status: valid, desc: "Repeated guards on one predicate's inputs mean the predicate asks the wrong question — fix the question.", keywords: "my_project's_own_skills_vanish_from_a_subdirectory suggestions_work_at_repo_root_but_not_in_src four_bugs_in_one_feature I_keep_patching_inputs fifth_guard is_cwd_a_project_root_or_inside_one walk_up_to_owning_project_root exact_root_equality_is_not_membership project_scoped_elements_dropped_at_once wrong_abstraction_not_bad_input cd_into_subfolder_lose_agents slug_computed_from_raw_cwd", ocd: 2026-08-29, lmd: 2026-08-29] DO NOT keep adding guards to a predicate's INPUTS after the second one fails, BECAUSE a recurring failure means the predicate asks the wrong QUESTION — here four guards (empty/relative/whitespace/root cwd) accumulated on a slug comparison that answered "is cwd EXACTLY a project root", so prompting from `<root>/src` still dropped the project's own elements. DO fix the question: resolve cwd to its OWNING root (walk up to the nearest `.claude/`) so the comparison matches how the data was keyed in the first place.
