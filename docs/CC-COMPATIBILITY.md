...
### v2.1.141 (2026-05-14)
- Hooks can emit terminal escape sequences (notifications, bells, window titles) via a new `terminalSequence` field in hook JSON output — PSS hook output stays informational (a `<pss-agents>` or `<pss-skills>` block, per the active suggestion mode), no adoption planned.
